...
class Metadata:
    ...
    @staticmethod
    def _get_files_with_metadata(root_folder: str) -> dict[str, str | None]:
        ...
        # Collect metadata files we successfully matched
        matched_metadata_files = {p for p in files_with_metadata.values() if p is not None}

        # Also consider "supplemental" metadata JSONs as valid known metadata files,
        # so we don't warn about them later.
        for f in files:
            for cand in Metadata._supplemental_metadata_candidates(f):
                if os.path.isfile(cand):
                    matched_metadata_files.add(cand)
    ...
    def _get_metadata_files(files: list[str]) -> dict[str, str | None]:
        ...
        files_with_metadata: dict[str, str | None] = {}

        for file in files:
            # 1) metadata is just appending the .json extension
            metadata_file = Metadata._sanitize_metadata_filename(f"{file}.json")
            if os.path.isfile(metadata_file):
                files_with_metadata[file] = metadata_file
                continue

            # 2) metadata is changing the extension to .json
            metadata_file = Metadata._change_extension(file)
            if os.path.isfile(metadata_file):
                files_with_metadata[file] = metadata_file
                continue

            # 3) Google Takeout often uses supplemental metadata filenames:
            #    <file>.<something>.json  (most common: .supplemental-metadata.json)
            metadata_file = Metadata._find_supplemental_metadata(file)
            if metadata_file is not None:
                files_with_metadata[file] = metadata_file
                continue

            # 4) Special case: foo(1).jpg -> foo.jpg(1).json
            ...
            # Try sanitized variant too (Takeout truncations)
            sanitized = Metadata._sanitize_metadata_filename(metadata_file)
            if os.path.isfile(sanitized):
                files_with_metadata[file] = sanitized
                continue
    ...
    @staticmethod
    def _supplemental_metadata_candidates(file: str) -> list[str]:
        """
        Candidates seen in the wild (and in your logs):
        - <file>.supplemental-metadata.json
        - <file>.supplemental-metad.json (truncated)
        - <file>.supplement.json
        """
        return [
            f"{file}.supplemental-metadata.json",
            f"{file}.supplemental-metad.json",
            f"{file}.supplement.json",
        ]

    @staticmethod
    def _find_supplemental_metadata(file: str) -> str | None:
        # First try exact candidates (no sanitizing)
        for cand in Metadata._supplemental_metadata_candidates(file):
            if os.path.isfile(cand):
                return cand

        # Then try sanitized candidates (handles Takeout truncation)
        for cand in Metadata._supplemental_metadata_candidates(file):
            sc = Metadata._sanitize_metadata_filename(cand)
